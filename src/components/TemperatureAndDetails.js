...
import { formatToLocaleTime, iconUrlFromCode } from '../services/weatherServicees'
...
function TemperatureAndDetails ({weather: {details, icon, temp, temp_min, temp_max, sunrise, sunset, speed, humidity, feels_like, timezone}}) {
    return (
        <div>
        <div className="flex flex-center justify-center py-6 text-xl text-cyan-300">
            <p>{details}</p>
        </div>
        ...
        <div className="flex flex-row items-center justify-center space-x-2 text-white text-sm py-3">
            <UilSun />
            <p className="font-light"> Rise: <span className='font-medium ml-1'> {formatToLocaleTime(sunrise, timezone, "hh:mm a")}</span></p>
            <p className='font-light'>|</p>

            <UilSunset />
            <p className="font-light"> set: <span className='font-medium ml-1'> {formatToLocaleTime(sunset, timezone, "hh:mm a")}</span></p>
            <p className='font-light'>|</p>
